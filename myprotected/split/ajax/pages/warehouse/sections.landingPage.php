<?php 
	...
	$itemsList = $zh->getSections($params);
	$totalItems = $zh->getSections($params,true);

	if ($itemsList) {
		foreach ($itemsList as $ri => &$row) {
			$row["fullname"] = $row["rack_name"].$row["row_name"].'-'.$row["name"];
		}
	}
	...
	$tableColumns = array(
						  ...
						  'Shelves'			=>	array('type'=>'text',		'field'=>'size_x'),
