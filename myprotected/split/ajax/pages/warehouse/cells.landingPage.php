<?php 
	...
	$itemsList = $zh->getCells($params);
	$totalItems = $zh->getCells($params,true);

	if ($itemsList) {
		foreach ($itemsList as $ri => &$cell) {
			$cell["fullname"] = $cell["rack_name"].$cell["row_name"].'-'.$cell["name"].'-'.$cell["x"].'-'.$cell["x"];
		}
	}
	...
	$tableColumns = array(
						  ...
						  'Row'				=>	array('type'=>'text',		'field'=>'row_name'),
						  'Section'			=>	array('type'=>'text',		'field'=>'section_name'),
