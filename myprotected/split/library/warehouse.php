...
class Warehouse extends BasicHelp {
	...
	public function getRowsList($rack_id = 1){
		$query = "SELECT M.*,
		(SELECT RACK.name FROM `osc_wh_racks` AS RACK WHERE  RACK.id = M.rack_id) as rack_name
	...
	public function getCellsItem($id) {
		$query = "
			SELECT M.*, 
			(SELECT S.row_id FROM `osc_wh_sections` AS S WHERE S.id = M.section_id LIMIT 1) AS row_id,
			(SELECT S.name FROM `osc_wh_sections` AS S WHERE S.id = M.section_id LIMIT 1) AS section_name,
			(SELECT ROW.rack_id FROM `osc_wh_rows` AS ROW WHERE ROW.id = row_id LIMIT 1) AS rack_id,
			(SELECT ROW.name FROM `osc_wh_rows` AS ROW WHERE ROW.id = M.row_id LIMIT 1) AS row_name,
			(SELECT RACK.name FROM `osc_wh_racks` AS RACK WHERE RACK.id = rack_id LIMIT 1) AS rack_name, 
			(SELECT RACK.warehouse_id FROM `osc_wh_racks` AS RACK WHERE RACK.id = rack_id LIMIT 1) AS warehouse_id, 
			(SELECT W.name FROM `osc_wh_warehouses` AS W WHERE W.id = warehouse_id LIMIT 1) AS warehouse_name
			FROM `osc_wh_cells` as M WHERE `id`='$id' LIMIT 1
		";
		$resultMassive = $this->rs($query);
		$result = ($resultMassive ? $resultMassive[0] : array());
		return $result;
	...
	public function getCells($params=array(),$typeCount=false) {
		$filter_and = "";
		if(isset($params['filtr']['massive'])) {
			foreach($params['filtr']['massive'] as $f_name => $f_value) {
				if($f_value < 0) continue;
	...
	public function getSectionsList($row_id){
		$query = "SELECT M.*, 
		(SELECT ROW.name FROM `osc_wh_rows` AS ROW WHERE ROW.id = M.row_id LIMIT 1) as row_name, 
		(SELECT ROW.rack_id FROM `osc_wh_rows` AS ROW WHERE ROW.id = M.row_id LIMIT 1) as rack_id, 
		(SELECT RACK.name FROM `osc_wh_racks` AS RACK WHERE RACK.id = rack_id LIMIT 1) as rack_name  
	...
	public function getFreeBoxesByType($type){
		// type == 1 ? small, type == 2 ? big
		//M.box_kind_id = '$type' AND 
		$q = "
			SELECT M.*,
			(SELECT CBR.id FROM `osc_wh_cell_box_ref` AS CBR WHERE CBR.box_id = M.id LIMIT 1) AS cell_box_ref_id
		 	FROM `osc_o_boxes` AS M ORDER BY M.article
		 ";
		return $this->rs($q);
	}
